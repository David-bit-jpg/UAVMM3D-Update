...
def val_rotation_enler(pred_q, gt_q, euler_seq='xyz', fold_180=True):
    """欧拉角 -> 四元数 -> 角度误差（度）。

    euler_seq 之前写死成 'zyx'，但 MAV6D 这条链路上的角度自始至终是 'xyz'：
    mav6d_*_dataset.py 用 as_euler('xyz') 生成 gt，object_encoder* 也按同样顺序
    编解码。按 'zyx' 去解释一组 'xyz' 角，得到的是另一个旋转，两边都错之后
    算出来的角度差并不等于真实角度差 —— 也就是说原来这个指标本身是错的。

    fold_180: 把大于 90° 的误差折算成 180° - err，等价于假设目标 180° 对称。
              这会让指标显著偏好，需要和不折算的数一起看，所以留成开关。
    """
    rotation_pred = R.from_euler(euler_seq, pred_q, degrees=False).as_quat()
    rotation_gt = R.from_euler(euler_seq, gt_q, degrees=False).as_quat()

    all_error = np.array([val_rotation(p, q) for p, q in zip(rotation_pred, rotation_gt)])
